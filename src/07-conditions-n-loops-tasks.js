...
function findFirstSingleChar(str) {
  ...
  // Шаг 1: Подсчет количества вхождений каждого символа
  for (const char of str) {
    charCount[char] = (charCount[char] || 0) + 1;
  }

  // Шаг 2: Поиск первого неповторяющегося символа
  for (const char of str) {
    if (charCount[char] === 1) {
      return char;
    }
  ...
  // Шаг 3: Если неповторяющийся символ не найден
  return null;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const interval = [Math.min(a, b), Math.max(a, b)].join(', ');
...
function getDigitalRoot(num) {
  while (num > 9) {
    num = [...num.toString()].map(Number).reduce((a, b) => a + b);
  }
  return num;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  // Шаг 1: Извлечение каталогов
  const directories = pathes.map((path) => path.split('/'));

  // Шаг 2: Поиск общего пути каталога
  const firstDirectory = directories[0];
  let commonPath = '';

  for (let i = 0; i < firstDirectory.length; i++) {
    const currentSegment = firstDirectory[i];
    if (directories.every((directory) => directory[i] === currentSegment)) {
  ...
  // Шаг 3: Возврат общего пути каталога
  return commonPath;
}
